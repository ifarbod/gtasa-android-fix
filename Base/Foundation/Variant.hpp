...
// Union for the possible variant values. Also stores non-POD objects such as String and math objects (excluding Matrix)
// which must not exceed 16 bytes in size (or 32 bytes in a 64-bit build.) Objects exceeding the limit are allocated on
// the heap and pointed to by _ptr.
struct VariantValue
{
    union
    {
