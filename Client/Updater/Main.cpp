...
void UI_CreateWindow()
{
    ...
    HWND rootWindow = CreateWindowExW(
        0, L"SAOUpdater", L"37.53% complete - Updating " MOD_NAME,
        WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, 0, WIDTH, HEIGHT, nullptr, nullptr, GetModuleHandle(NULL), nullptr);
    ...
    HWND static1 = CreateWindowExW(WS_EX_TRANSPARENT, L"static", L"static1", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 25, 15, 396, 25, rootWindow, 0, GetModuleHandle(NULL), 0);
    SendMessage(static1, WM_SETFONT, (WPARAM)font, 0);
    SetWindowText(static1, L"Copying 2,417 items from Games to Games");

    HWND static2 = CreateWindowExW(WS_EX_TRANSPARENT, L"static", L"static2", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 25, 33, 396, 25, rootWindow, 0, GetModuleHandle(NULL), 0);
    SendMessage(static2, WM_SETFONT, (WPARAM)font3, 0);
    SetWindowText(static2, L"Paused - 5% complete");

    HWND progressBar = CreateWindowEx(WS_EX_WINDOWEDGE | WS_EX_CLIENTEDGE, L"msctls_progress32", 0, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS,
        25, 63, 396, 15, rootWindow, 0, GetModuleHandle(NULL), 0);
    ...
    HWND yesButton = CreateWindowEx(0, L"button", L"Yes", WS_CHILD | WS_CLIPSIBLINGS, 270, 85, 75, 23, rootWindow, 0, GetModuleHandle(NULL), 0);
    SendMessage(yesButton, WM_SETFONT, (WPARAM)font, 0);

    HWND cancelNoButton = CreateWindowEx(0, L"button", L"Cancel", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 347, 85, 75, 23, rootWindow, 0, GetModuleHandle(NULL), 0);
    SendMessage(cancelNoButton, WM_SETFONT, (WPARAM)font, 0);

    HWND static3 = CreateWindowEx(0x20, L"static", L"static3", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 25, 83, 240, 22, rootWindow, 0, GetModuleHandle(NULL), 0);
    SendMessage(static3, WM_SETFONT, (WPARAM)font, 0);
    SetWindowText(static3, L"13.00 / 17.89MB (9.7 MB/s)");

    HWND static4 = CreateWindowEx(0x20, L"static", L"static4", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 25, 101, 240, 22, rootWindow, 0, GetModuleHandle(NULL), 0);
    SendMessage(static4, WM_SETFONT, (WPARAM)font, 0);
    SetWindowText(static4, L"32:57 | -01:37");

    HWND static5 = CreateWindowEx(0x20, L"static", L"static5", WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS, 25, 119, 396, 22, rootWindow, 0, GetModuleHandle(NULL), 0);
    ...
    MoveWindow(rootWindow, (width - WIDTH) / 2, (height - HEIGHT) / 2, wndRect.right - wndRect.left + 1, wndRect.bottom - wndRect.top + 1, TRUE);
